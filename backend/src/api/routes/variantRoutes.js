...
router.post('/:productId/variants', protect, async (req, res, next) => {
    try {
...
// @access  Private/Seller
router.put('/:productId/variants/:variantId', protect, async (req, res, next) => {
    try {
        const product = await Product.findByPk(req.params.productId);
        if (!product) {
            return res.status(404).json({ message: 'Product not found' });
...
// @access  Private/Seller
router.delete('/:productId/variants/:variantId', protect, async (req, res, next) => {
    try {
        const product = await Product.findByPk(req.params.productId);
        if (!product) {
            return res.status(404).json({ message: 'Product not found' });
