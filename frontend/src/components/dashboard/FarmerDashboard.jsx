  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <StatsCard icon="fas fa-seedling" iconColor="#1C4B2D" iconBg="#f0fdf4" label="My Products" value={stats?.totalProducts || 0} />
        <StatsCard icon="fas fa-coins" iconColor="#f59e0b" iconBg="#fffbeb" label="Total Revenue" value={`KES ${(stats?.totalSales || 0).toLocaleString()}`} />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
        ...
        <div style={{ background: '#fff', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', border: '1px solid #f0f0f0' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
            ...
            <Link to="/orders" style={{ fontSize: '13px', color: '#1C4B2D', textDecoration: 'none', fontWeight: 600 }}>View All →</Link>
          </div>
              ...
              {recentOrders.map(order => (
                ...
                >
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: '13px', fontWeight: 700, color: '#1C4B2D' }}>KES {parseFloat(order.total_amount || 0).toLocaleString()}</div>
        ...
        <div style={{ background: '#fff', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', border: '1px solid #f0f0f0' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
            <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 700, color: '#111827' }}>
              <i className="fas fa-seedling" style={{ marginRight: '8px', color: '#1C4B2D' }} />
              My Products
            </h3>
            <Link to="/products" style={{ fontSize: '13px', color: '#1C4B2D', textDecoration: 'none', fontWeight: 600 }}>Manage →</Link>
          </div>
          {myProducts.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '30px', color: '#6b7280' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {myProducts.map(product => (
                <div key={product.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px', borderRadius: '8px', background: '#f9fafb' }}>
                  <div style={{ width: '40px', height: '40px', borderRadius: '8px', background: '#1C4B2D22', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
                    {product.image_url ? (
                      <img src={product.image_url} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    ) : (
                      <i className="fas fa-seedling" style={{ color: '#1C4B2D', fontSize: '16px' }} />
                    )}
                  </div>
